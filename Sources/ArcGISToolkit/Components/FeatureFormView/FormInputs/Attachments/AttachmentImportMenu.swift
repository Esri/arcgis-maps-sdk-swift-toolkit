...
struct AttachmentImportMenu: View {
    ...
    var body: some View {
        ...
        .task(id: newAttachmentData) {
            ...
            _ = await element.addAttachment(
                name: "Attachment \(element.attachments.count + 1)",
                image: newAttachmentData
            )
        ...
        .fileImporter(isPresented: $fileImporterIsShowing, allowedContentTypes: [.image]) { result in
            switch result {
            case .success(let url):
                guard let data = FileManager.default.contents(atPath: url.path) else {
                    Logger.featureFormView.error("File picker data was empty")
                    return
                }
                newAttachmentData = data
            case .failure(let error):
                Logger.featureFormView.error("Error importing from file importer: \(error)")
            }
        }
